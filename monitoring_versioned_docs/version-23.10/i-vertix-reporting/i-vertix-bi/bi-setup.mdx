...
To monitor your Business Intelligence system, we created a new plugin pack which you can download from the [*Plugin Store*](../../monitoring-resources/monitoring-basics/plugin-packs).
...
- <ImageCounter>1</ImageCounter> Monitoring database server IP address
   ...
   - In case you have a [remote database server architecture](../../installation/before-you-start/architecture#remote-database-management-architecture), use the **IP of the remote database server** (must be reachable by the BI server)
