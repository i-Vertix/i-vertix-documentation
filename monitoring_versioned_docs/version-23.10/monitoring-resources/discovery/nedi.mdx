...
The job will run instantly with the provided configuration and selected entry points.
...
* <ImageCounter num={6} /> <strong>Sync with Monitoring</strong>: sync all or sync selected devices to the monitoring configuration
...
* <ImageCounter num={7} /> <strong>Link existing Hosts</strong>: link all or link selected devices with already existing hosts

  only effective for devices with status *existing (ip)* and/or status *conflict (name)*

* <ImageCounter num={8} /> <strong>Other actions</strong>
...
* <ImageCounter num={9} /> <strong>Regenerate Proposals</strong>: re-applies template rules to all devices, useful after changing template rules or including devices

#### <ImageCounter num={10} /> Device List
...
The shown templates were assigned by the rule engine because a template rule matched with the device's information.
...
You can verify the **Relation Root** status also from the [network device list](./nedi#-discovered-device-list).
...
* <ImageCounter num={3} /> <strong>Change Shadow IT Status</strong>: change the shadow IT status of selected nodes
...
This status helps you to categorize any discovered component in your network.
Together with the custom *Note* you can set, you can keep track and updates of any desired, suspect or undesired devices in your network.
