...
i-Vertix IT Monitoring Engine supports optional detection of hosts and services that
...
Whenever i-Vertix IT Monitoring Engine checks the status of a host or service, it will
...
-   If the service was not previously flapping and 31% is equal to or
    greater than the high flap threshold, i-Vertix IT Monitoring Engine considers the
    service to have just started flapping.
-   If the service was previously flapping and 31% is less than the low
    flap threshold, i-Vertix IT Monitoring Engine considers the service to have just
    stopped flapping.
...
select a scheduler (i-Vertix IT Monitoring Engine). In the **Check Options** tab, you
...
If you enable the flapping detection for a scheduler (i-Vertix IT Monitoring Engine),
