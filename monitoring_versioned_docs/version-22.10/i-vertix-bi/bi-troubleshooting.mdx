...
There could be several reasons which may lead into connection issues to your i-Vertix BI server:
...
Please beware that deactivating the maintenance status during an actual system update can lead to **inconsistent** or **suddendly aborted report job executions**
...
If the ETL job is taking longer as expected (*\<amount of days synced\>* \* 30 (\*estimated minutes for one day\*)) you should check the ETL log files on the i-Vertix BI server. 
