...
plugin that actually executes the monitoring commands on a i-Vertix Poller.
...
i-Vertix discovery engine to fetch additional assets to be monitored. Host
...
Monitoring Connectors on your i-Vertix platform are managed by the Monitoring Connector Manager
user interface. Monitoring Connectors may be updated or new Monitoring Connectors may be added to
the i-Vertix online library on a weekly basis. These are accessed from the
...
(Applications, i-Vertix, Cloud, Database, Hardware, Network, Operating System,
