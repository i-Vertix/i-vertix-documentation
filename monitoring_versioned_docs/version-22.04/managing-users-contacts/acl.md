...
3. To add users (contacts) or contact groups to the access group, use the **Linked Contacts/Linked Contact Groups** table. (Select the user(s) you want in the **Available** column, and then click **Add**. The user(s) is moved to the **Selected** column.)
    ...
    >
    > Groups created in the IT Monitoring interface should not have the same name as LDAP groups to avoid problems.
...
The access filters on resources allow you to define which objects (hosts, host groups, services and service groups) users will be able to see in the IT Monitoring interface.
...
the case, users will have to access the page via a direct link
 (autologin, etc.).
    - Whenever a new IT Monitoring module is created with a web interface accessible
