...
The Report Store is similar to our [Plugin Store](../../monitoring-resources/plugin-packs.mdx) which is used to install and update plugin packs.
...
To access the Report Store, make sure you already have [configured the i-Vertix Store credentials](../../quick-start-guide/plugin-store-configuration.mdx).
...
To access the Report Store, head over to <Breadcrumb crumbs={["Administration", "Stores", "Report Store"]} />.
...
You can find the list of missing prerequisites in the [Report Template Detail](./bi-report-store.mdx#report-template-detail).
