...
- [Database update](./bi-settings.mdx#-versions)
- [Configure SMTP for i-Vertix BI](./bi-settings.mdx#smtp-configuration)
- [Add timeperiods to be used in BI](./bi-settings.mdx#-timeperiods)
- [Exclude pollers from BI](./bi-settings.mdx#-pollers)
- [Exclude hostgroups from BI](./bi-settings.mdx#-hostgroups)
...
please search the [Report generation failing troubleshooting section](./bi-troubleshooting.mdx#report-generation-is-failing) for *ETL job still running*`.
...
The API token was generated and presented during the [setup process of the i-Vertix BI server](./bi-setup.mdx#configure-bunsiness-intelligence-services-and-database).
To re-generate a new API token, re-run the setup on the *i-Vertix Business Intelligence* machine (you will be asked if you want to generate a new token).
After you have generated a new token, update the <ImageCounter num={2} disableMargin /> existing record.

### <ImageCounter num={3} /> Monitoring Database Connection

Reconnecting the database is necessary when the i-Vertix BI IP address has changed.

This database connection is used by the *i-Vertix Business Intelligence* server to access the monitoring database.
For the connection a dedicated database user was created which is restricted to only access from the defined i-Vertix Business Intelligence Server IP address.

:::danger

Please change the <ImageCounter num={1} /> *Business Intelligence Server IP* and **save** the settings **before** reconnecting the database.
...
[4th step of the module setup process](./bi-setup.mdx#4th-step-monitor-the-i-vertix-business-intelligence-server).
