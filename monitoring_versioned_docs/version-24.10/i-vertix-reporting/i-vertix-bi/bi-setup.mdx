...
- <ImageCounter>1</ImageCounter> Monitoring database server IP address
   - Most probably your monitoring database is present on the monitoring server it self, in that case please **insert the monitoring server IP** (must be reachable by the BI server)
