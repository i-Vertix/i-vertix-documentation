...
please read the [netflow infrastructure chapter](./netflow-overview.md#main-components).
...
### Prepare the NetFlow Analyzer System
...
To make sure everything works the way it should, have a look at the below port table to understand, which ports are used for communication between the different systems.

Additional to the below ports, the **[default monitoring port mappings](../installation/before-you-start/technical-information.md#tables-of-platform-flows-centralpoller) between *Central* and *Poller* are also used**.

| Source System              | Destination System         | Protocol | Port                                                    | Description                                                               |
| -------------------------- | -------------------------- | -------- | ------------------------------------------------------- | ------------------------------------------------------------------------- |
| Monitoring Central         | NTA System    | SSH      | TCP 22                                                  | Used to deploy NetFlow Collectors                                         |
| Monitoring Central         | NTA System    | HTTPS    | TCP 8443                                                | Used to query data from the NTA API                                       |
| Poller (NetFlow Collector) | NTA System    | ZMQ      | TCP 5556-5656 (*NetFlow Analyzer Interface Port*)       | Used to send flows from the collector to the centralized NetFlow Analyzer |
| NetFlow Device             | Poller (NetFlow Collector) | NetFlow  | default UDP 6363 or UDP 2055 (*NetFlow Collector Port*) | Used to send flows from a device to the NetFlow Collector                 |
...
Here is a guide on [how to configure the network](../installation/setup-central-poller/network-configuration.mdx).
...
You can also add a new dedicated poller which only functions as a *NetFlow collector*. Please follow [these steps to attach a new poller to the monitoring system](../installation/attach-a-poller-to-a-central-server.mdx).
