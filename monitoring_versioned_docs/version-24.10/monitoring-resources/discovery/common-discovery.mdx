...
:::info
The features described in this section require a recent version of the monitoring system.
If they are not visible in your installation, please update to the latest available release.

Also it may be required to update to the latest plugin version in the [Plugin Store](../../monitoring-resources/plugin-packs.mdx)
to get access to the individual job types.
:::
...
This step of the wizard configures basic information for the job as well as anything regarding access, 
...
This section is different for each discovery type. The image below shows an example of a discovery job related to *Azure*.
...
Please note that not every discovery type returns the latitude and longitude coordinates.
...
* <ImageCounter num={3} /> **Conditions**: configured conditions are applied to every discovery result and decide,
  whether the template rule will be applied to the result/host or not. **In case no condition is present, the template rule will be applied to every result automatically**
...
a host get updated when you select results to update the monitoring configuration from.
...
All update-options should be available to be selected/removed from the profile.
:::
...
The first section in this tab covers the generated main host information, such as:
...
7. Choose or create a resource group containing the resources you want to monitor/discover
...
13. Retrieve the *Tenant ID*
  ...
  * Navigate to **Overview** and copy the *Tenant ID*
...
The collected data is stored locally and reused by discover and monitoring processes.
...
[create the new *Proxmox VE credential*](#configuration-parameters) using the new user credentials.
