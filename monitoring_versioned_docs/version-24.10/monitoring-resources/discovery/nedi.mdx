...
They come along with the plugins installed from the [Plugin Store](../../monitoring-resources/plugin-packs.mdx).
...
1. **Download the relevant plugin** from the [Plugin Store](../../monitoring-resources/plugin-packs.mdx) and install it
...
* **Status**: [discovery status](./nedi.mdx#status-discovery-status)
...
* **Shadow IT status**: current shadow IT status for this node - can be changed from the [Nodes tab](./nedi.mdx#nodes)
...
The <ImageCounter num={2} /> items on the map are filtered by the <ImageCounter num={1} /> [device list filters](./nedi.mdx#-device-filters). 
...
You can verify the **Relation Root** status also from the [network device list](./nedi.mdx#-device-list).
