---
id: updating-services-dynamically
title: Update services dynamically
---
...
It is possible to modify certain parts of a service's configuration dynamically.
This feature is especially useful if certain service configurations need to change frequently based on different conditions.
For example, you may want to switch between various notification configurations for multiple services when a service is part of a specific group.
...
![Rulesets](../../assets/monitoring-resources/updating-dynamically/dynamic_service_configuration.png)
...
- <ImageCounter num={2} /> options to filter the list:
    - filter by rule name
    - filter by macro names (macros to set on the service)
    - filter by contactgroups (to set on the service)
    - filter by contacts (to set on the service)
...
- <ImageCounter num={5} /> a button to simulate all rulesets and to analyze which services will be updated
- <ImageCounter num={6} /> a button to apply all rulesets manually
- <ImageCounter num={7} /> the possibility to [reorder rules](./updating-services-dynamically.mdx#order-rulesets)
...
![Add Rule](../../assets/monitoring-resources/updating-dynamically/dynamic_service_sidebar.png)
...
- <ImageCounter num={3} /> select the access groups for the rule; only services which are accessible by the selected 
...
- <ImageCounter num={4} /> choose a service template
- <ImageCounter num={5} /> choose a severity

### <ImageCounter num={6} /> Set Service Macros
...
- <ImageCounter num={7} /> enable or disable notifications
- <ImageCounter num={8} /> choose the desired notification options to specify when a notification should be sent

#### <ImageCounter num={9} /> Contacts

> Setting contacts is optional

Select contacts that will be assigned to matched services.
When `Incremental` is checked, selected contacts are **added** to services; when unchecked, selected contacts are **overwritten** on services.
...
When `Incremental` is checked, selected contact groups are **added** to services; when unchecked, selected contact groups are **overwritten** on services.
...
![Add Rule](../../assets/monitoring-resources/updating-dynamically/service_condition_simple.png)
...
This behavior ensures that only services from a certain organization are affected by the ruleset (MSP compliance).
...
This is especially useful when you want to target services more specifically—such as when they belong to a certain organization,
...
Rules are applied from top to bottom in the displayed order, so keep this behavior in mind if multiple rules affect the same services.
...
![Conflicts](../../assets/monitoring-resources/updating-dynamically/service_simulation_conflicts.png)

A special orange entry appears in the simulation results when conflicts are detected. The information is as follows:

- <ImageCounter num={1} /> the header contains details about the number of conflicting rules and services
- <ImageCounter num={2} /> clicking the header opens a detailed view with a list of conflicts grouped by field
- <ImageCounter num={3} /> the <code>Field</code> column shows the service field in conflict
...
- <ImageCounter num={6} /> the <code>Services</code> column lists all services affected by the conflict (Host name - Service name)

</TabItem>
<TabItem value="results" label="Results">

![Results](../../assets/monitoring-resources/updating-dynamically/service_simulation.png)
...
- <ImageCounter num={1} /> the header displays the rule name, the number of fields set by the rule, and the number of services affected
...
- <ImageCounter num={4} /> a list of all services affected by this rule; these services will be updated with the specified field values upon applying the rule
...
Keep in mind that rule application relies on order. For example, if multiple rules affect a service,
it may not be necessary to adjust a rule if the **last** value applied is the desired one.

Using the [simulation conflict detail](./updating-services-dynamically.mdx?simulation=conflicts#test-your-rulesets) view during
...
Handling conflicts can be complex, as you need absolute clarity on the final value that will be set on a service.
...
After applying, ensure that you [export the corresponding poller configurations](../export-configuration.mdx) for your rules to take full effect.
