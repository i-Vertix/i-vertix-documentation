...
as explained in the [Plugin Store Configuration Guide](../quick-start-guide/plugin-store-configuration.mdx).
...
The procedure can be started by clicking on the button and, after its completion, we need to [export the monitoring configuration to the `Pollers`](./export-configuration.mdx).
...
See the link [Deploy the configuration](./export-configuration.mdx) for the full explanation on how to export the configuration to the *Pollers*.
