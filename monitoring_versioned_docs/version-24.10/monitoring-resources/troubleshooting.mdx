...
Navigate to <Breadcrumb crumbs={["Administration", "ACL", "Menu Access"]} /> and select the *Menu ACL* to which you want to grant access.

![Menu ACL](../assets/monitoring-resources/troubleshooter/acl.png)

Expand the `Monitoring` → `Troubleshooter` section and select the features
(<ImageCounter num={1} disableMargin /> `Monitoring Plugin`, <ImageCounter num={2} disableMargin /> `SNMP`, <ImageCounter num={3} disableMargin /> `Utility Commands`)
