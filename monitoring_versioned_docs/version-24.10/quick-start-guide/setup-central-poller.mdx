...
After you have made sure that your organization fulfills the [requirements](./requirements.mdx) you can now start with the setup of your *i-Vertix Monitoring* solution.
This guide only describes the minimum steps to install and configure the system. For a more detailed installation, check the complete [Installtion Guide](../installation/before-you-start/platform.md).

:::note

The following steps assume you already know which architecture you are about to install and how much resources you need for each virtual appliance as listed in the [requirements](./requirements.mdx).
...
Check our [import a virtual appliance](../installation/setup-central-poller/import-virtual-appliance.md) guide if you are not 100% sure how to import the appliance correctly.
...
Our [*Network configuration*](../installation/setup-central-poller/network-configuration.mdx) article explains how to use the network manager (nmtui).
...
You can now proceed with the [*Plugin Store Configuration*](./plugin-store-configuration.mdx) in case you don't have to set up *i-Vertix Pollers* or if you want to do that later.
...
Please read and run through the complete [Attach a Poller to the Central Management](../installation/attach-a-poller-to-a-central-server.mdx) process to add the new pollers to your monitoring system.
