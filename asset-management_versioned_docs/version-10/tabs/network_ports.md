...
This tab, named **Network ports** allows to manage the network ports
attached to a computer. They can be brought up by the automatic
inventory or added manually

![List of network ports](../assets/tabs/images/network-port-view.png)

A network port represents the output of a network interface on a given
hardware. A port is characterized by a number and a name. The
information that can be viewed is:
...
This information can be uploaded via the automatic inventory. It can
also be added/modified manually.
...
You will be asked to provide a certain amount of information. Once you
have filled in the details, click on **+ Add**

![add new port](../assets/tabs/images/network-port-add.png)

### Additional configuration of new port

Once added, you can view/modify/add a certain amount of information from
the various fields that have just been added.

![view new port details](../assets/tabs/images/network-port-details.png)

### Metrics

In the **Metrics** tab, you can see the machine's network consumption

![view new port metrics](../assets/tabs/images/network-port-metrics.png)

### Network Name

You can associate a network name via this tab or create a new one by
clicking on **+ Add**. Go to
[Network Name](../modules/configuration/dropdowns/network-name.md) for more details.

![network port name](../assets/tabs/images/network-port-name.png)
...
being defined by a name, an optional comment and a VLAN number. Tags are
available for VLAN.

![view new port vlan](../assets/tabs/images/network-port-vlan.png)
...
![add new VLAN](../assets/tabs/images/network-port-vlan-add.png)

You can select your new VLAN in the dropdown list.

### Locks

A locked field is a manually modified field. The automatic inventory
will no longer modify this field, unless you unlock it. The automatic
inventory will no longer handle this item, unless you unlock it.

![Network port locks](../assets/tabs/images/network-port-locks.png)
...
![Network port locks](../assets/tabs/images/network-port-unlock.png)

### Historical

This field references all the changes that have been made to the network
port, whether manually or via the automatic inventory

![Network port historical](../assets/tabs/images/network-port-historical.png)
